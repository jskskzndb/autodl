...
class PHD_DecoderBlock(nn.Module):
    def __init__(self, in_channels, out_channels, use_dcn=True, dropout_p=0.1):
        super().__init__()
    ...
    def forward(self, x):
        ...
        out = self.fusion(feat_local, feat_global)
...
class Up_PHD(nn.Module):
    def __init__(self, in_channels, out_channels, skip_channels=0, use_dcn=True, dropout_p=0.1):
        super().__init__()
        self.up = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
        conv_in_channels = in_channels + skip_channels
        self.conv = PHD_DecoderBlock(in_channels=conv_in_channels, out_channels=out_channels, use_dcn=use_dcn, dropout_p=dropout_p)
...
class UNet_CNext_PHD(nn.Module):
    def __init__(self, n_classes=1, cnext_type='convnextv2_base', use_dcn=True, dropout_p=0.1, **kwargs):
        super().__init__()
        ...
        self.spatial_encoder = timm.create_model(
            ...
            features_only=True, 
            out_indices=(0, 1, 2, 3),
            drop_path_rate=0.4  # <--- 🔥 [关键修改] Base 模型必须开到 0.4！
        )
        ...
        self.up1 = Up_PHD(c4, c3, skip_channels=c3, use_dcn=use_dcn, dropout_p=dropout_p)
        
        # Up 2: Input=c3, Skip=c2 -> Out=c2
        self.up2 = Up_PHD(c3, c2, skip_channels=c2, use_dcn=use_dcn, dropout_p=dropout_p)
        
        # Up 3: Input=c2, Skip=c1 -> Out=c1
        self.up3 = Up_PHD(c2, c1, skip_channels=c1, use_dcn=use_dcn, dropout_p=dropout_p)
