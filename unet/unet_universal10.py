...
# ================================================================
# 🔥 [新增] ICCV 2019 CARAFE 上采样算子 (PyTorch Native Implementation)
# ================================================================

class CarafeUpsample(nn.Module):
    """
    修正版 CARAFE (维度对齐优化)
    解决了 F.unfold 与预测核维度不匹配的 RuntimeError
    """
    def __init__(self, in_channels, out_channels, scale_factor=2, k_encoder=3, k_up=5):
        ...
        # 预测核的通道数必须严格等于 scale^2 * k_up^2
        self.kernel_predictor = nn.Conv2d(
            self.comp, 
            (scale_factor ** 2) * (k_up ** 2), 
    ...
    def forward(self, x):
        ...
        kernels = self.kernel_predictor(x_comp) # [B, S^2 * K^2, H, W]
        
        # 使用 PixelShuffle 将核分布到目标分辨率 [B, K^2, H*S, W*S]
        kernels = F.pixel_shuffle(kernels, S) 
        kernels = F.softmax(kernels, dim=1) # 在 K^2 维度归一化
        
        # --- Step 2: 特征重组 (重点修复区) ---
        x_in = self.dim_proj(x)
        
        # 2.1 提取邻域: [B, C*K^2, H*W]
        # 使用特定的 padding=K//2 确保输出中心对齐
        x_unfold = F.unfold(x_in, kernel_size=K, padding=K // 2, stride=1)
        
        # 2.2 调整维度以便进行“内容感知”加权
        # 将 x_unfold 变为 [B, C, K^2, H, W]
        x_unfold = x_unfold.view(B, self.out_channels, K**2, H, W)
        
        # 2.3 关键步骤：同步上采样特征邻域图
        # 将 [B, C, K^2, H, W] 放大到 [B, C, K^2, H*S, W*S]
        # 这样每一个目标像素位置 (H*S, W*S) 都有一个对应的 K^2 邻域
...
class UniversalUNet(nn.Module):
    def __init__(self, n_classes=1, cnext_type='convnextv2_tiny', pretrained=True, decoder_type='phd', 
        ...
        # 🔥 [核心修改] 替换原来的 final_up (4x Interpolation) 为级联 CARAFE
        # s_dims[0] 通常是 96
        # 1. 插入 IGS 模块 (新增)
        self.img_stem = ImageGuidedStem(in_ch=3, out_ch512=16, out_ch256=32)
        # Stage 4: 128 -> 256
        # 使用 CARAFE 上采样 + 一个轻量级残差块修补特征
        self.up4_carafe = CarafeUpsample(in_channels=s_dims[0], out_channels=32, scale_factor=2, k_up=5)
        
        self.up4_fuse = nn.Sequential(
            nn.Conv2d(64, 64, kernel_size=3, padding=1, groups=64, bias=False), # Depthwise: 空间对齐
            nn.Conv2d(64, 32, kernel_size=1, bias=False),                      # Pointwise: 通道融合降维
            nn.BatchNorm2d(32),
            nn.ReLU(inplace=True)
        )
        ...
        self.outc = nn.Conv2d(16 + 16, n_classes, kernel_size=1)
    ...
    def forward(self, x):
        ...
        d4 = torch.cat([d4, f256_skip], dim=1) # 拼接 IGS 提取的 256 尺度细节
        # 重点：先投影降维，再做残差卷积
        d4 = self.up4_fuse(d4)                 # 3x3 空间+通道融合
        d4 = d4 + self.up4_refine(d4)          # 残差细化
        
        # 256 -> 512
        d5 = self.up5_carafe(d4)
        d5 = torch.cat([d5, f512_skip], dim=1) # 拼接 IGS 提取的 512 尺度细节
        d5 = self.up5_fuse(d5)
        logits = self.outc(d5)
