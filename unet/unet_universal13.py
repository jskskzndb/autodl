...
[Universal Model] 全能型 UNet (Dynamic Version - ASPP & Edge Attention Edition)
架构特点:
  ...
  4. Decoder: Dynamic PHD Decoder (动态原型解码器，极速版)
  5. Image-Guided Stem (IGS): 搭载 ASPP-Lite，专门针对小目标进行多尺度高分辨率特征重组
  6. Cascade CARAFE: 内容感知上采样配合 DW-Fusion 融合
  7. Edge Consistency: 在最终 512 尺度引入联合注意力门控 (d5 * boundary_weight) 削减毛刺
"""
...
# ================================================================
# 4. 解码器组件: Dynamic PHD Decoder (移除跨层共鸣，恢复极速版)
# ================================================================
...
class PHD_DecoderBlock_Pro(nn.Module):
    ...
    def forward(self, x):
        x = self.align(x)
        # 🔥 [Bug 修复保留] 遍历 ModuleList 时，分别提取交互块和前馈网络
        for i, layer_module in enumerate(self.layers):
            interaction_block = layer_module[0]
            ffn_block = layer_module[1]
            
            x_inter = interaction_block(x)
...
class EdgeAwareDualScaleGIS(nn.Module):
    """ 
    [终极单流特化版] 双尺度对称物理边缘先验 GIS
    特色: 
      1. 512 和 256 尺度独立进行无损 Sobel 测绘。
      2. 采用 Concat + 1x1 降维进行最安全的早期特征融合。
      3. 双尺度残差边缘门控 (Residual Edge Gating) 强制修正边界。
    """
    def __init__(self, in_ch=3, out_ch512=16, out_ch256=32):
        super().__init__()
        
        # =========================================================
        # 0. 物理边缘算子 (死算子，不参与梯度更新)
        # =========================================================
        ...
        # 1. 512 尺度专区 (目标: 16 通道)
        # =========================================================
        # 输入: 原图 3 通道 + 边缘 1 通道 = 4 通道
        self.initial_512 = nn.Sequential(
            nn.Conv2d(in_ch + 1, out_ch512, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(out_ch512), nn.ReLU(True)
        )
        self.aspp_512 = DepthwiseASPP_Lite(out_ch512, out_ch512, mid_ch=out_ch512)
        self.gate_512 = nn.Sequential(
            nn.Conv2d(out_ch512, 1, kernel_size=1, bias=False),
            nn.Sigmoid()
        )

        # =========================================================
        # 2. 桥梁：下采样特征图 (16通道 -> 32通道)
        # =========================================================
        ...
        # 3. 256 尺度专区 (目标: 32 通道)
        # =========================================================
        # 输入: f256_base 32 通道 + 边缘 1 通道 = 33 通道。需要 1x1 卷积对齐回 32 通道。
        self.align_256 = nn.Sequential(
            nn.Conv2d(out_ch256 + 1, out_ch256, kernel_size=1, bias=False),
            nn.BatchNorm2d(out_ch256), nn.ReLU(True)
        )
        self.aspp_256 = DepthwiseASPP_Lite(out_ch256, out_ch256, mid_ch=out_ch256)
        self.gate_256 = nn.Sequential(
            nn.Conv2d(out_ch256, 1, kernel_size=1, bias=False),
            nn.Sigmoid()
        )

    def forward(self, x):
        # 防御性代码：确保算子和输入在同一设备且数据类型一致 (抵抗 AMP 干扰)
        ...
        # 阶段 A：提取 512 物理边缘
        # ---------------------------------------------------------
        gray_512 = 0.299 * x[:, 0:1] + 0.587 * x[:, 1:2] + 0.114 * x[:, 2:3]
        grad_x_512 = F.conv2d(gray_512, sx, padding=1)
        grad_y_512 = F.conv2d(gray_512, sy, padding=1)
        edge_512 = torch.sqrt(grad_x_512 ** 2 + grad_y_512 ** 2 + 1e-6)

        # ---------------------------------------------------------
        # 阶段 B：512 尺度流水线 (Concat -> ASPP -> Gate)
        # ---------------------------------------------------------
        x_512_concat = torch.cat([x, edge_512], dim=1)           # [B, 4, 512, 512]
        f512_feat = self.aspp_512(self.initial_512(x_512_concat)) # [B, 16, 512, 512]
        g512 = self.gate_512(f512_feat)                          # [B, 1, 512, 512]
        f512_skip = f512_feat * (1.0 + g512)                     # 残差放大，准备跳跃连接

        # ---------------------------------------------------------
        # 阶段 C：提取 256 物理边缘 (双线性抗锯齿插值 -> Sobel)
        # ---------------------------------------------------------
        gray_256 = F.interpolate(gray_512, scale_factor=0.5, mode='bilinear', align_corners=False)
        grad_x_256 = F.conv2d(gray_256, sx, padding=1)
        grad_y_256 = F.conv2d(gray_256, sy, padding=1)
        edge_256 = torch.sqrt(grad_x_256 ** 2 + grad_y_256 ** 2 + 1e-6)

        # ---------------------------------------------------------
        # 阶段 D：256 尺度流水线 (Down -> Concat -> Align -> ASPP -> Gate)
        # ---------------------------------------------------------
        f256_base = self.down_feat(f512_skip)                    # [B, 32, 256, 256]
        f256_concat = torch.cat([f256_base, edge_256], dim=1)    # [B, 33, 256, 256]
        f256_aligned = self.align_256(f256_concat)               # 重新压回 [B, 32, 256, 256]
        
        f256_feat = self.aspp_256(f256_aligned)                  # [B, 32, 256, 256]
        g256 = self.gate_256(f256_feat)                          # [B, 1, 256, 256]
        f256_skip = f256_feat * (1.0 + g256)                     # 残差放大，准备跳跃连接

        return f512_skip, f256_skip

# ================================================================
# 7. 主模型: UniversalUNet (ASPP & Edge Attention Final)
# ================================================================
...
class UniversalUNet(nn.Module):
    def __init__(self, n_classes=1, cnext_type='convnextv2_tiny', pretrained=True, decoder_type='phd', 
        ...
        print(f"🤖 [Dynamic Universal Model] Initialized:")
        print(f"   - Encoder: {cnext_type}")
        print(f"   - IGS Mode: ASPP-Lite Multi-scale Prior")
        print(f"   - Edge Mode: Joint Edge Attention Gating")
        ...
        self.img_stem = EdgeAwareDualScaleGIS(in_ch=3, out_ch512=16, out_ch256=32)
    ...
    def forward(self, x):
        ...
        d4 = self.up4_fuse(d4)                 # 32ch
        d4 = d4 + self.up4_refine(d4)          
        
        d5 = self.up5_carafe(d4)
        d5 = torch.cat([d5, f512_skip], dim=1) # 32ch (16+16)
        d5 = self.up5_fuse(d5)                 # DW平滑 -> 输出 32ch
        
        # 🔥 [新增] 联合注意力门控
        # 联合 f512_skip (细节) 和 d5 (语义) 算出一张纯粹的边缘注意力图
        boundary_weight = self.edge_attn(torch.cat([f512_skip, d5], dim=1)) # [B, 1, 512, 512]
        # 残差相乘，把边缘处的特征值剧烈放大
        d5 = d5 + d5 * boundary_weight
        
        logits = self.outc(d5)                 # 最终分类输出
        ...
        if self.training:
            ...
            # 显式监督选项：把 boundary_weight 送出，方便你在 train02.py 里算 Loss
            # 注意：这会让 outputs 的长度增加。你可以选择用真实的边缘图对它算 BCE Loss。
            outputs.append(boundary_weight) 
